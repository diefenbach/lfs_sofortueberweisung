...
class SofortUeberweisungPaymentMethodProcessor(PaymentMethodProcessor):
    ...
    def process(self):
        return {
            ...
            % (
                ...
                settings.SOFORTUEBERWEISUNG_PROJECT_ID,
                self.order.number,
                self.order.price,
            ),
        }
    ...
    def get_pay_link(self):
        return (
            "https://www.sofortueberweisung.de/payment/start?user_id=%s&project_id=%s&reason_1=Bestellnummer %s&amount=%s&currency=EUR"
            % (
                settings.SOFORTUEBERWEISUNG_USERID,
                settings.SOFORTUEBERWEISUNG_PROJECT_ID,
                self.order.number,
